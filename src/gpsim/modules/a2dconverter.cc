...
//------------------------------------------------------
ADCON0::ADCON0(Processor *pCpu, const char *pName)
    : SfrReg(pCpu, pName ),
 ...
 */
void ADCON0::setAdres(SfrReg *new_adres)
{
    adres = new_adres;
}
/*
 ...
 */
void ADCON0::setAdresLow(SfrReg *new_adresl)
{
    adresl = new_adresl;
}
/*
...
void ADCON0::start_conversion(void)
{
    ...
    uint64_t fc = cpu->currentCycle() +( 2 * Tad) / p_cpu->get_ClockPerInstr();
...
void ADCON0::put_conversion(void)
{
    ...
    if( dNormalizedVoltage < 0 ) dNormalizedVoltage = 0;
...
void ADCON0::callback(void)
{
    ...
    case AD_ACQUIRING:
        ...
        future_cycle = cpu->currentCycle() +( m_nBits * Tad)/p_cpu->get_ClockPerInstr();
    ...
    case AD_CONVERTING:
        ...
        value.put(value.get() &( ~GO_bit)); // Clear the GO/!DONE flag.
        set_interrupt();
...
// ADCON0
//
ADCON0_DIF::ADCON0_DIF(Processor *pCpu, const char *pName )
    : ADCON0(pCpu, pName )
{
}
...
ADCON1_16F::ADCON1_16F(Processor *pCpu, const char *pName )
    : ADCON1(pCpu, pName ), FVR_chan(99)
{
    valid_bits = 0x70;
}
...
// ADCON1
//
ADCON1::ADCON1(Processor *pCpu, const char *pName )
    : SfrReg(pCpu, pName ),
...
void ADCON1::setADCnames()
{
    ...
    for(uint i = 0; i < m_nAnalogChannels; i++)
    {
        if( (diff &( 1 << i)) && m_AnalogPins[i] != &AnInvalidAnalogInput)
        {
            ...
            else m_AnalogPins[i]->AnalogReq( this, false, "?" /*m_AnalogPins[i]->getPin().name_str.c_str()*/ );
        }
    }
    mIoMask = new_mask;
}
...
double ADCON1::getChannelVoltage( uint channel )
{
    ...
    if( channel < m_nAnalogChannels )
    {
        ...
        else        // use voltage reference
        {
            ...
            if( voltage < 0.0 )
            {
                cout << "ADCON1::getChannelVoltage channel " << channel << " not a configured input\n";
                voltage = 0.0;
            }
        }
    }
...
{
    uint channel = Vrefhi_position[cfg_index];
    if( channel < m_nAnalogChannels)
    {
        /// return getChannelVoltage(Vrefhi_position[cfg_index]); // SimulIDE: Modified for 887 ADC
        PinModule* pm = m_AnalogPins[channel];
        if( pm != &AnInvalidAnalogInput ) return pm->getPin().get_nodeVoltage();
    }
    return cpu->get_Vdd();
}
...
{
    uint channel = Vreflo_position[cfg_index];
    if( channel < m_nAnalogChannels)
    {
        /// return getChannelVoltage(Vreflo_position[cfg_index]); // SimulIDE: Modified for 887 ADC
        PinModule* pm = m_AnalogPins[channel];
        if( pm != &AnInvalidAnalogInput ) return pm->getPin().get_nodeVoltage();
    }
    return 0.0;
...
ANSEL_H::ANSEL_H(Processor *pCpu, const char *pName )
    : SfrReg(pCpu, pName ),
...
ANSEL_P::ANSEL_P(Processor *pCpu, const char *pName )
    : SfrReg(pCpu, pName ),
...
// ADCON0_10
//
ADCON0_10::ADCON0_10(Processor *pCpu, const char *pName )
    : ADCON0(pCpu, pName )
{
...
// register
//
ADCON0_12F::ADCON0_12F(Processor *pCpu, const char *pName )
    : ADCON0(pCpu, pName )
{
    GO_bit = GO;        //ADCON0 and ADCON0_10 have GO flag at different bit
    valid_bits = 0xdf;
...
ANSEL_12F::ANSEL_12F(Processor *pCpu, const char *pName )
    : SfrReg(pCpu, pName ),
      adcon1(0)
{
}
...
// register
//
ADCON0_32X::ADCON0_32X(Processor *pCpu, const char *pName )
    : ADCON0(pCpu, pName )
{
    GO_bit = GO;        //ADCON0 and ADCON0_10 have GO flag at different bit
    valid_bits = 0xff;
 ...
 */
    ...
    if( (new_value & ADON) != ADON) new_value &= ~GO_bit; // If ADON is clear GO cannot be set
    ...
    if(new_value & ADON)
    {
        ...
        if((new_value & ~old_value) & GO_bit) // The 'GO' bit is being turned on, which is request to initiate and A/D conversion
        {
            start_conversion();
        }
    }
...
//--------------------------------------------------
//
FVRCON::FVRCON( Processor *pCpu, const char *pName , uint bitMask, uint alwaysOne )
    : SfrReg( pCpu, pName ),
...
double FVRCON::compute_VTemp(uint fvrcon)
{
    ...
    {
        Vt = 0.659 -(  cpu14->m_cpu_temp + 40.) * 0.00132;
        ret = cpu->get_Vdd() -( (fvrcon&TSRNG)?4.:2.) * Vt;
        if( ret < 0.0)
        {
            ret = -1.;
...
//--------------------------------------------------
//
DACCON0::DACCON0(Processor *pCpu, const char *pName , uint bitMask, uint bit_res)
    : SfrReg(pCpu, pName ),
...
void DACCON0::set_dacoutpin( bool output_enabled, int chan, double Vout )
{
    ...
    if( output_pin[chan]) out_pin =( IO_bi_directional_pu*) &(output_pin[chan]->getPin());
    else return;
    ...
    if( output_enabled )
    {
        ...
        {
            std::string pin_name = name_str.substr(0, 4);
    ...
    {
        output_pin[chan]->AnalogReq( this, false, "?" /*output_pin[chan]->getPin().name_str.c_str(*/ );
...
DACCON1::DACCON1(Processor *pCpu, const char *pName , uint bitMask, DACCON0 *_daccon0)
    : SfrReg(pCpu, pName ),
...
//
ADCON2_DIF::ADCON2_DIF(Processor *pCpu, const char *pName )
    : SfrReg(pCpu, pName )
{
}
...
//
ADCON2_TRIG::ADCON2_TRIG(Processor *pCpu, const char *pName )
    : SfrReg(pCpu, pName ), valid_bits(0xf0), m_adcon0(0)
{
}
