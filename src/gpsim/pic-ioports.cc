...
PicPortRegister::PicPortRegister(Processor *pCpu, const char *pName, uint numIopins, uint enableMask)
    : PortRegister( pCpu, pName, numIopins, false ), m_tris(0)
{
...
public:
    ...
    virtual char getState() { return m_register ? m_register->get3StateBit(m_bitMask) : '?'; }
...
PicTrisRegister::PicTrisRegister(Processor *pCpu, const char *pName,
                                 PicPortRegister *_port, bool bIgnoreWDTResets, uint enableMask)
    : SfrReg( pCpu, pName ),
      m_port(_port ),
      m_EnableMask( enableMask),
      m_bIgnoreWDTResets( bIgnoreWDTResets)
{
    if( m_port)  m_port->setTris(this);
}
...
PicPSP_TrisRegister::PicPSP_TrisRegister(Processor *pCpu, const char *pName ,
                                         PicPortRegister *_port, bool bIgnoreWDTResets)
    : PicTrisRegister(pCpu, pName,_port,bIgnoreWDTResets)
{
}
...
void PicPSP_TrisRegister::put(uint new_value)
{
    ...
    if( !( new_value & PSP::PSPMODE)) fixed = 0;
    else                              fixed = value.data & mask;

    value.data =( new_value & ~mask) | fixed;
    if( m_port) m_port->updatePort();
}

void PicPSP_TrisRegister::put_value(uint new_value) // used by gpsim to change register value
{
...
PicPortBRegister::PicPortBRegister(Processor *pCpu, const char *pName, INTCON *pIntcon,
                                   uint numIopins,
                                   uint enableMask,
                                   INTCON2 *pIntcon2,
                                   INTCON3 *pIntcon3)
    : PicPortRegister( pCpu, pName  , numIopins, enableMask),
      m_bRBPU(false),
      m_bIntEdge(true),
      m_bsRBPU(0),
      m_pIntcon(pIntcon),
...
void PicPortBRegister::put( uint new_value )
{
    ...
    if( value.data == drivingValue ) return; // Try it
...
public:
    ...
    void setSink(bool b)
    {
        if( m_pPortB) m_pPortB->setRBPU(b);
    }
};

void PicPortBRegister::assignRBPUSink(uint bitPos, SfrReg *pSFR)
{
    if( pSFR && !m_bsRBPU)
    {
...
PicPortGRegister::PicPortGRegister(Processor *pCpu, const char *pName  ,
    ...
    : PicPortBRegister(pCpu, pName  , pIntcon, numIopins, enableMask),
      m_pIntcon(pIntcon), m_pIoc(pIoc), intf_bit(0) // intf_bit was 2, now default to 0, 12f6xx uses 2
{
...
PicPSP_PortRegister::PicPSP_PortRegister(Processor *pCpu, const char *pName  ,
                                         /*const char *port_name,*/
                                         uint numIopins,
                                         uint enableMask)
    : PortRegister(pCpu, pName  ,numIopins, false), m_tris(0), m_psp(0)
{
    setEnableMask(enableMask);
}
...
{
    if( m_psp && m_psp->pspmode()) return(m_psp->psp_get());
...
PicLatchRegister::PicLatchRegister(Processor *pCpu, const char *pName,
                                   PortRegister *_port,
                                   uint enableMask)
    : SfrReg(pCpu, pName  ),
