...
static const char* Circuit_properties[] = {
    QT_TRANSLATE_NOOP("App::Property","Speed per"),
    QT_TRANSLATE_NOOP("App::Property","Speed sps"),
    QT_TRANSLATE_NOOP("App::Property","Simu Step nS"),
...
QString Circuit::loc()
{
    ...
    else if( m_lang == French )     locale = "fr";
    else if( m_lang == German )     locale = "de";
    else if( m_lang == Hungarian )  locale = "hu";
...
QString Circuit::getCompId( QString name ) // Get Component Id out of Pin Id
{
...
void Circuit::loadDomDoc( QDomDocument* doc )
{
    ...
    if( circuit.hasAttribute( "Simu_Step_nS" )) setStepNs( circuit.attribute("Simu_Step_nS").toDouble() );
...
uint64_t Circuit::circSpeed() { return m_simulator.stepsPS(); }
...
{
    double spsReal = m_simulator.stepsPS();
