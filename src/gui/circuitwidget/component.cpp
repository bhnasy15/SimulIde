...
static const char* Component_properties[] = {
    ...
    QT_TRANSLATE_NOOP("App::Property","itemtype")
};
...
{
    if( this->parentItem() ) // Pass event to parent
    {
    ...
    if( event->button() == Qt::LeftButton )
    {
        ...
        if( m_selMainCo )  // Used when creating Boards to set this as main component
        {
            ...
            m_selMainCo = false;
            /// CircuitView::self()->viewport()->setCursor( Qt::ArrowCursor );
            update();
            return;
        }
...
void Component::remove()
{
    ...
    for( Pin* pin : m_pin )   // Remove connectors attached
    {
        if( pin && pin->connector() ) pin->connector()->remove();
    }
