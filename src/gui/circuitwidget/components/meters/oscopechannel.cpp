...
void OscopeChannel::updateStep()
{
    ...
    if( m_period > 10 )  // We have a wave
    {
        ...
        {
            freq = (m_freq+9*1e9/((double)m_totalP/(double)(m_numMax-1)))/10;
            m_totalP  = 0;
            m_numMax  = 0;
        }
        if( freq<20 ) setVChanged();
    }
    else{
        setVChanged();
        freq = 0;
    ...
    if( m_period > 10 )  // Do we still have a wave?
    {
        ...
        uint64_t stepSize = Simulator::self()->m_stepNano*1e3;//stepSize();
...
void OscopeChannel::setVChanged()
{
