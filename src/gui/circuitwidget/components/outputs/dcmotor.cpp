...
void DcMotor::updateStep()
{
    if( Simulator::self()->isRunning() )
    {
        ...
        if( m_updtTime ) m_speed = m_delta/(m_updtTime/1e9);
...
void DcMotor::updatePos()
{
    ...
    m_delta += (m_LastVolt/m_voltNom)*(duration/1e9);
    m_LastVolt = m_ePin[0]->getVolt() - m_ePin[1]->getVolt();
}
