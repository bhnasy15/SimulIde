...
MainWindow::MainWindow()
          : QMainWindow()
          , m_settings( QStandardPaths::standardLocations( QStandardPaths::DataLocation).first()
                        +"/simulide.ini", QSettings::IniFormat, this )
{
    ...
    /*QString ICs = "/mnt/software/simulide/simulide_lauchpad/simulide_0.4.14/build/executables/SimulIDE_0.4.15-SR7/share/simulide/data/ICs";
    QDir IcDir = QDir( ICs );
    QStringList dirList = IcDir.entryList();

    for( QString dir :dirList )
    {
        if( dir.startsWith(".") ) continue;
        QString dirPath = IcDir.absoluteFilePath( dir );
        QDir SimiDir = QDir( dirPath );
        SimiDir.setNameFilters( QStringList( "*.simu" ) );
        QStringList simList = SimiDir.entryList( QDir::Files );
        for( QString sim :simList )
        {
            QString si = sim;
            if( !si.toLower().contains("test") ) continue;
            QString simPath = SimiDir.absoluteFilePath( sim );
            CircuitWidget::self()->clear();
            Circuit::self()->loadCircuit( simPath );
            Circuit::self()->saveCircuit( simPath );
        }
    }*/
}
MainWindow::~MainWindow(){ }
