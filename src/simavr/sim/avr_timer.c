...
{
    return p->io.avr->data[p->comp[compi].r_ocr] |( p->comp[compi].r_ocrh ?
...
static avr_cycle_count_t avr_timer_comp( avr_timer_t* p, avr_cycle_count_t when, uint8_t comp )
{
    ...
    uint64_t comp_cycles = p->comp[comp].comp_cycles;
    uint32_t clear = 0;
    uint32_t set   = 1;

    if( p->wgm_kind == wgm_fc_pwm )
    {
        if( p->countUp == 0 ) // Counting Down
        {
            clear = 1;
            set   = 0;
        }
    ...
    switch( mode )
    {
        ...
        case avr_timer_com_toggle: // Toggle OCnA on compare match
            ...
            ///// TODO: not all modes in all timers allow toggling
        ...
        case avr_timer_com_clear:
            avr_raise_irq( irq, clear );
            break;

        case avr_timer_com_set:
            avr_raise_irq( irq, set );
            break;
    }
    return p->tov_cycles? 0 : comp_cycles? comp_cycles : 0;
...
{
    if( p->wgm_kind == wgm_fc_pwm ) return; // Phase Correct: Do nothing on TOV
...
static avr_cycle_count_t avr_timer_clock( struct avr_t* avr, avr_cycle_count_t when, void* param )
{
    avr_timer_t* p =( avr_timer_t*)param;

    //AVR_LOG(avr, LOG_TRACE, "%s Timer%c tick, tcnt=%i\n", __func__, p->name, p->tov_base);

    p->ext_clock_flags |= EXTCLK_FLAG_STARTED;
    ...
    */
    ...
    {
        if( p->wgm_kind != wgm_ctc )
        {
            if(( p->Wgm_mode.top == wgm_reg_ocra) &&( compi == 0) ) continue; // ocra used to define TOP
        }
        if( p->comp[compi].comp_cycles
        &&( p->tov_base == p->comp[compi].comp_cycles) )
        {
            dispatch[compi]( avr, avr->cycle, param );
            //if( p->wgm_kind == wgm_ctc) p->tov_base = 0;
        }
    }

    switch( p->wgm_kind )
    {
        case wgm_fc_pwm:    // in the avr_timer_write_ocr comment "OCR is not used here" - why?

        //case avr_timer_wgm_pwm:
            if(( p->ext_clock_flags & EXTCLK_FLAG_REVDIR) != 0 )
            {
                --p->tov_base;
                if( p->tov_base == 0 ) // overflow occured
                {
                    p->ext_clock_flags &= ~EXTCLK_FLAG_REVDIR; // restore forward count direction
                    overflow = 1;
                }
            }else { if( ++p->tov_base >= p->tov_top )
                { p->ext_clock_flags |= EXTCLK_FLAG_REVDIR; }// prepare to count down
            }
            break;
        ...
        case wgm_fast_pwm:
            {
                ...
                if( ++p->tov_base > max ) // overflow occured
                {
                    p->tov_base = 0;
                    overflow = 1;
                }
        ...
        case wgm_ctc:
            ...
            {
                overflow = 1;
                if     ( p->Wgm_mode.top == wgm_reg_icr)  avr_raise_interrupt( avr, &p->icr );
                else if( p->Wgm_mode.top == wgm_reg_ocra) avr_raise_interrupt( avr, &p->comp[0].interrupt );
            }
            else if( p->tov_base > p->tov_top ) { p->tov_base = 0; }
            break;

        default:
            if( ++p->tov_base > p->tov_top ) // overflow occured
            {
                p->tov_base = 0;
                overflow = 1;
            }
    ...
    if( overflow )
    {
        for( int compi=0; compi<AVR_TIMER_COMP_COUNT; compi++ )
        {
            if( p->comp[compi].comp_cycles )
            {
                //if( p->mode.top == wgm_reg_ocra && compi == 0 )
                //    continue;
    ...
    uint16_t tcnt = p->tov_base;
    avr->data[p->r_tcnt] = tcnt;
    if( p->r_tcnth) avr->data[p->r_tcnth] = tcnt >> 8;
    return avr->cycle+p->cs_div_value;
}


static void avr_timer_irq_ext_clock( struct avr_irq_t* irq, uint32_t value, void * param)
{
    avr_timer_t * p =( avr_timer_t *)param;
    avr_t * avr = p->io.avr;

    if( (p->ext_clock_flags & EXTCLK_FLAG_VIRT) || !p->tov_top)
        return;            // we are clocked internally( actually should never come here)

    int bing = 0;
    if( p->ext_clock_flags & EXTCLK_FLAG_EDGE) { // clock on rising edge
        if(!irq->value && value) bing++;
    }
    else {    // clock on falling edge
        if( irq->value && !value) bing++;
    }
    if( bing) avr_timer_clock( avr, value, param );
...
static avr_cycle_count_t avr_timer_tov( struct avr_t* avr, avr_cycle_count_t when, void* param )
{
    ...
    if( ((p->ext_clock_flags &( EXTCLK_FLAG_AS2 | EXTCLK_FLAG_TN)) != 0)
    ...
    for( int compi=0; compi<AVR_TIMER_COMP_COUNT; compi++ )
    {
        uint64_t comp_cycles = p->comp[compi].comp_cycles;
        uint64_t next_match =  p->comp[compi].ocr_cycles;

        if( comp_cycles )
        {
            if( p->wgm_kind == wgm_fc_pwm )
            {
                if( p->countUp ) next_match = p->tov_cycles-next_match; // Counting Up
                avr_cycle_timer_register( avr, next_match, dispatch[compi], p );
            }
            else
            {
                if( comp_cycles < p->tov_cycles
                 && comp_cycles >= (avr->cycle-when))
                {
                    avr_timer_comp_on_tov( p, when, compi );
                    avr_cycle_timer_register( avr, comp_cycles-(avr->cycle-next), dispatch[compi], p );
                }
    ...
    if( p->wgm_kind == wgm_fc_pwm )
    {
        if( p->countUp ) p->countUp = 0;  // prepare to count down
        else             p->countUp = 1;  // restore forward count direction
    }
...
{
    avr_t* avr = p->io.avr;
    if( !(p->ext_clock_flags &( EXTCLK_FLAG_TN | EXTCLK_FLAG_AS2))
      ||( p->ext_clock_flags & EXTCLK_FLAG_VIRT) )
    {
        if( p->tov_cycles)
        {
    ...
    }else { if( p->tov_top ) return p->tov_base; }
...
static inline void
avr_timer_cancel_all_cycle_timers( struct avr_t* avr, avr_timer_t* timer, const uint8_t clear_timers )
{
    if( clear_timers )
    {
        for( int compi=0; compi<AVR_TIMER_COMP_COUNT; compi++ )
...
static void avr_timer_tcnt_write( struct avr_t* avr, avr_io_addr_t addr, uint8_t v, void* param)
{
    ...
    else p->tov_base = tcnt; // clocked externally
}

static void avr_timer_configure( avr_timer_t* p, uint32_t prescaler, uint32_t top, uint8_t reset )
{
    p->tov_top = top;
    ...
    uint8_t as2 = p->ext_clock_flags & EXTCLK_FLAG_AS2;
    uint8_t use_ext_clock = as2 ||( p->ext_clock_flags & EXTCLK_FLAG_TN);
    uint8_t virt_ext_clock = use_ext_clock &&( p->ext_clock_flags & EXTCLK_FLAG_VIRT);

    if( !use_ext_clock )
    {
        if( prescaler != 0 ) resulting_clock =( float)avr->frequency/prescaler;

        uint64_t tov_cycles = prescaler*(top+1);
        if( p->wgm_kind == wgm_fc_pwm ) tov_cycles = prescaler*top;

        p->tov_cycles = tov_cycles;
        p->tov_cycles_fract = 0.0f;
    ...
    else
    {
        ...
        {
            if( prescaler != 0 ) resulting_clock = p->ext_clock/prescaler;
    ...
    for( int compi=0; compi<AVR_TIMER_COMP_COUNT; compi++ )
    {
        avr_timer_comp_t* comp = &(p->comp[compi]);
        if( !comp->r_ocr ) continue;

        uint32_t ocr = _timer_get_ocr( p, compi );
        //uint32_t comp_cycles = clock *( ocr + 1);

        uint32_t comp_cycles;
        if( virt_ext_clock ) comp_cycles =( uint32_t)((float)avr->frequency/p->ext_clock*prescaler*(ocr+1));
        else                 comp_cycles = prescaler*(ocr+1);
        ...
        if( p->trace &(avr_timer_trace_compa << compi) )
        {
            ...
            {
                printf("%s-%c clock %f top %d OCR%c %d\n", __FUNCTION__, p->name, resulting_clock, top, 'A'+compi, ocr);
            }
            else
            {
        ...
        if( ocr <= top )
        {
            comp->comp_cycles = comp_cycles;
            comp->ocr_cycles  = comp_cycles;

            if( p->trace &( avr_timer_trace_compa << compi)) printf( "TIMER: %s-%c %c %.2fHz = %d cycles\n", __FUNCTION__, p->name, 'A'+compi, resulting_clock /( ocr+1),( int)comp_cycles);
        }
        // Get control / Release simulide pins
        uint8_t mode = avr_regbit_get( avr, comp->com) ;
        avr_raise_irq( &p->io.irq[TIMER_IRQ_OUT_PWM0+compi], mode );
    }
    ...
    if( !use_ext_clock || virt_ext_clock )
    {
        if( p->tov_cycles > 1 )
        {
            if( reset)
            {
                ...
                ////avr_cycle_timer_register( avr, prescaler, avr_timer_clock, p );
            ...
            else
            {
                ...
                ////avr_cycle_timer_register( avr, prescaler-(avr->cycle-p->tov_base), avr_timer_clock, p );
                // calling it once, with when == 0 tells it to arm the A/B/C timers if needed
                p->tov_base = 0;
                avr_timer_tov( avr, orig_tov_base, p );
            }
        }
    }else { if( reset) p->tov_base = 0; }

    if( reset)
    {
        avr_ioport_getirq_t req = { .bit = p->ext_clock_pin };

        if( avr_ioctl( p->io.avr, AVR_IOCTL_IOPORT_GETIRQ_REGBIT, &req) > 0 )
        {
            if( use_ext_clock && !virt_ext_clock ) // got an IRQ for the Tn input clock pin
            {
                if( p->trace) AVR_LOG(p->io.avr, LOG_TRACE, "%s: timer%c connecting T%c pin IRQ %d\n", __FUNCTION__, p->name, p->name, req.irq[0]->irq);

                avr_irq_register_notify( req.irq[0], avr_timer_irq_ext_clock, p );
            }
            else
            {
                if( p->trace) AVR_LOG(p->io.avr, LOG_TRACE, "%s: timer%c disconnecting T%c pin IRQ %d\n", __FUNCTION__, p->name, p->name, req.irq[0]->irq);
...
static void avr_timer_reconfigure( avr_timer_t* p, uint8_t reset )
{
    avr_t * avr = p->io.avr;

    avr_timer_cancel_all_cycle_timers( avr, p, 1 ); // cancel everything

    uint16_t top;
    if     ( p->Wgm_mode.top == wgm_reg_ocra ) top = _timer_get_ocr( p, AVR_TIMER_COMPA );
    else if( p->Wgm_mode.top == wgm_reg_icr )  top = _timer_get_icr( p );
    else                                       top = p->wgm_size;

    avr_timer_configure( p, p->cs_div_value, top, reset );
}

static void avr_timer_write_icr( struct avr_t* avr, avr_io_addr_t addr, uint8_t v, void* param )
{
    avr_timer_t* timer = (avr_timer_t*)param;

    avr_core_watch_write( avr, addr, v );

    if( timer->Wgm_mode.top == wgm_reg_icr )
    {
        uint16_t top = v | ( timer->r_tcnth ?( timer->io.avr->data[timer->r_icrh] << 8) : 0);
        avr_timer_configure( timer, timer->cs_div_value, top, 0 );
    }
}
...
static void avr_timer_write_ocr( struct avr_t* avr, avr_io_addr_t addr, uint8_t v, void* param )
{
    ...
    uint8_t comp_n = comp->number;

    if( (comp_n == 0) && (timer->Wgm_mode.top == wgm_reg_ocra) )
    ...
    else
    {
        ...
        uint64_t ocr_cycles = prescaler*ocr;

        comp->ocr_cycles  = ocr_cycles;
        comp->comp_cycles = ocr_cycles;

        if( ( timer->wgm_kind == wgm_fc_pwm ) // We are in Phase Correct mode
          &&( timer->countUp == 0 ) )              // Counting Down
    ...
    switch( timer->wgm_kind )
    {
        ...
        case wgm_fc_pwm:
            if( timer->Wgm_mode.top == wgm_reg_icr )  // ICR is the top, update comp_cycles
            {
                uint32_t ocr = _timer_get_comp_ocr( avr, comp );
                uint32_t prescaler = timer->cs_div_value;
                comp->comp_cycles = prescaler*ocr;
            }
            else avr_timer_reconfigure( timer, 0 ); // if OCRA or Max is the top, reconfigure needed
            break;
...
static void avr_timer_write( struct avr_t * avr, avr_io_addr_t addr, uint8_t v, void * param)
{
    ...
    if( new_cs != cs || new_mode != mode || new_as2 != as2 )
    {
        ...
        p->ext_clock_flags &= ~(EXTCLK_FLAG_TN  | EXTCLK_FLAG_EDGE | EXTCLK_FLAG_AS2 | EXTCLK_FLAG_STARTED);

        if( p->ext_clock_pin.reg
        &&( p->cs_div[new_cs] == EXTCLK_CHOOSE))
        {
            // Special case: external clock source chosen, prescale divider irrelevant.
            p->cs_div_value = 1;
            p->ext_clock_flags |= EXTCLK_FLAG_TN |( new_cs & EXTCLK_FLAG_EDGE);
        }
        else
        {
            ...
            if( new_as2 )
            {
                ...
                p->ext_clock_flags |= EXTCLK_FLAG_AS2 | EXTCLK_FLAG_EDGE;
            }
        }

    /* mode */
        p->Wgm_mode = p->wgm_op[new_mode];
        p->wgm_kind = p->Wgm_mode.kind;
        p->wgm_size = (1 << p->Wgm_mode.size)-1;
 ...
 * pending interrupts.
 */
static void avr_timer_write_pending(
        struct avr_t * avr,
        avr_io_addr_t addr,
        uint8_t v,
        void * param)
{
    ...
    if( v & p->overflow.raised.mask ) avr_clear_interrupt_if( avr, &p->overflow, ov );
    if( v & p->icr.raised.mask )      avr_clear_interrupt_if( avr, &p->icr, ic );
...
static void avr_timer_irq_icp( struct avr_irq_t * irq, uint32_t value, void * param)
{
    ...
    if( p->Wgm_mode.top == wgm_reg_icr ) return; // input capture disabled when ICR is used as top
        ...
        void * io_param)
{
    ...
    else if( ctl == AVR_IOCTL_TIMER_SET_VIRTCLK(p->name))
    {
        ...
        if( !new_val)
        {
            avr_ioport_getirq_t req_timer_clock_pin = { .bit = p->ext_clock_pin };
...
static void avr_timer_reset( avr_io_t* port)
{
    ...
    /*for( int compi = 0; compi < AVR_TIMER_COMP_COUNT; compi++ )
    {
        ...
        avr_ioport_getirq_t req = { .bit = p->comp[compi].com_pin };
        ...
        if( avr_ioctl(port->avr, AVR_IOCTL_IOPORT_GETIRQ_REGBIT, &req) > 0) {
            ...
            /// This raises a PORT irq which is wrong
            /// Simulide should connect to timer irq: port->irq[TIMER_IRQ_OUT_COMP + compi]
    ...
    avr_ioport_getirq_t req = { .bit = p->icp  };
    ...
    p->ext_clock_flags &= ~(EXTCLK_FLAG_STARTED | EXTCLK_FLAG_TN | EXTCLK_FLAG_AS2 | EXTCLK_FLAG_REVDIR);
...
void avr_timer_init( avr_t* avr, avr_timer_t* p)
{
    ...
    if( p->r_icr )
        avr_register_io_write( avr, p->r_icr, avr_timer_write_icr, p );
