...
void Simulator::runGraphicStep2()
{
    ...
    {
        m_realSpeed = ( m_tStep-m_lastStep )*1e9/deltaRefTime;
        CircuitWidget::self()->setRate( (m_realSpeed*100)/(1e4*m_stepsPerus) /*m_stepsPS*/ );
        m_lastStep    = m_tStep;
...
void Simulator::runExtraStep( uint64_t timeNs )
{
    m_circTime = timeNs;
...
void Simulator::startSim()
{
    ...
    solveMatrix(); // m_matrix.printMatrix();
